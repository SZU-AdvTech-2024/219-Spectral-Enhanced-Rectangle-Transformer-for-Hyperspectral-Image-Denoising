...
def train_options(parser):
    """
    传入一个命令行接口对象，为其定义接收的命令行参数，返回该对象
    """
    ...
    parser.add_argument('--loss', type=str, default='l2',
                        ...
                        choices=['l1', 'l2', 'smooth_l1', 'ssim', 'l2_ssim', 'l2_sam', 'cons', 'cons_l2'])
    ...
    parser.add_argument('--gpu-ids', type=str, default='0,1,2,3', help='gpu ids')
...
class Engine(object):
    def __init__(self, opt):
        self.prefix = opt.prefix  # 前缀
        self.opt = opt          # 命令行参数
        self.net = None         # 网络模型
        self.optimizer = None   # 优化器
        self.criterion = None   # 损失函数，默认使用 MSELoss（均方误差损失）
        self.basedir = None     # 程序生成文件保存的根目录
        self.iteration = None   # 每轮学习的迭代次数
        self.epoch = None       # 学习次数
        self.best_psnr = None   # 最佳信噪比
        self.best_loss = None   # 最好损失
    ...
    def __setup(self):
        ...
        if not os.path.exists(self.basedir):
            os.makedirs(self.basedir) # 创建checkpoints目录
        ...
        cuda = not self.opt.no_cuda  # 获取命令行参数 --no-cuda 对应数值
        self.device = 'cuda' if cuda else 'cpu'
        print('Cuda Acess: %d' % cuda)
        if cuda and not torch.cuda.is_available():
            raise Exception("No GPU found, please run with --no-cuda")

        # 为当前CPU设置随机种子，所有后续的随机操作（如初始化权重、打乱数据集、随机选择等）都会基于这个种子产生相同的结果
        torch.manual_seed(self.opt.seed)
        if cuda:
            torch.cuda.manual_seed(self.opt.seed)  # 为当前GPU设置随机种子
    ...
    def __step(self, train, inputs, targets, sigma=None):
        ...
        if self.get_net().bandwise:
            ...
            loss_data += loss.item()
        if train:
            # 裁剪（限制）模型参数的梯度，以避免梯度爆炸问题
            # self.opt.clip 指定了梯度的最大范数（norm）。如果梯度的范数超过这个值，梯度将按比例缩减，以使其不超过 self.opt.clip
            ...
            for batch_idx, (inputs, targets) in enumerate(valid_loader):
                ...
                for i in range(width // patch_size):
                    for j in range(height // patch_size):
                        sub_image = inputs[:, :, i * patch_size:(i + 1) * patch_size,
                                    j * patch_size:(j + 1) * patch_size]
                        input_patch[num] = sub_image
                        targets_patch[num] = targets[:, :, i * patch_size:(i + 1) * patch_size,
                                             j * patch_size:(j + 1) * patch_size]
                        num += 1
                if not self.opt.no_cuda:
                ...
                for i in range(width // patch_size):
                    for j in range(height // patch_size):
                        ...
                        result_patch[num]
                        img[:, i * patch_size:(i + 1) * patch_size, j * patch_size:(j + 1) * patch_size] = img_patch[
                            num]
                        num += 1
                for k in range(31):
                    psnr.append(10 * np.log10((h * w) / sum(sum((result[k] - img[k]) ** 2))))
    ...
    def validate(self, valid_loader, name, patch_size=64):
        ...
        if not self.opt.no_log:
            wandb.log(
                {'val_loss_epoch': avg_loss, 'val_psnr_epoch': avg_psnr, 'val_sam_epoch': avg_sam, 'epoch': self.epoch})
        ...
        with torch.no_grad():
            for batch_idx, inputs in enumerate(test_loader):
                ...
                if savedir:
                    ...
                    savepath = join(savedir, basename(dataset.filenames[batch_idx]).split('.')[0],
                                    self.opt.arch + '.mat')
