...
try:
    from .load_tif import load_tif_img, is_tif_file
except:
    from load_tif import load_tif_img, is_tif_file
...
class AddNoiseNoniid_v2(object):
    ...
    def __call__(self, img):
        bwsigmas = np.reshape((np.random.rand(img.shape[0]) * (self.max_sigma - self.min_sigma) + self.min_sigma),
                              (-1, 1, 1))
...
class DataLoaderVal_TIF(Dataset):
    def __init__(self, data_dir, ratio=50, target_transform=None):
        ...
        self.noisy_filenames = [os.path.join(data_dir, 'input{}'.format(ratio), x) for x in noisy_files if
                                is_tif_file(x)]

        self.clean = [torch.from_numpy(np.float32(load_tif_img(self.clean_filenames[index]))) for index in
                      range(len(self.clean_filenames))]
        self.noisy = [torch.from_numpy(np.float32(load_tif_img(self.noisy_filenames[index]))) for index in
                      range(len(self.noisy_filenames))]
...
def addNoise2ICVL():
    ...
    srcdir = '/home2/szl/sert/data/ICVL/icvl_val_512/'
    noise_sig = [10,30,50,70]
    noisemodel = AddNoiseNoniid(noise_sig)
    # noisemodel = AddNoiseBlindv1(10,70)
    deadlinemodel = AddNoiseDeadline()
    impulsemodel = AddNoiseImpulse()
    stripemodel = AddNoiseStripe()
    dstdir ='/home2/szl/sert/data/ICVL/icvl_val_512_complex'
    mkdir(dstdir)
    c = 0
    # dstdir = '/home2/szl/sert/data/ICVL/icvl_test_512_gaussian/10_70'
    ...
    for filename in os.listdir(srcdir):
        ...
        noisyhsi = deadlinemodel(noisyhsi)
        noisyhsi = impulsemodel(noisyhsi)
    ...
    #     dstdir = '/home2/szl/sert/data/ICVL/icvl_val_512_gaussian/' + str(sigma)
    #     mkdir(dstdir)
