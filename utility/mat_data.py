...
def create_mat_dataset(datadir, fnames, newdir, matkey, func=None, load=h5py.File):
    ...
    for i, fn in enumerate(fnames):
        ...
        try:
            ...
            try:
                Image.fromarray(np.array(data_hwc*255,np.uint8)[:,:,20]).save('/home2/szl/sert/data/ICVL/icvl_val_512_png/{}.png'.format(os.path.splitext(fn)[0]))
...
def create_icvl_sr():
    basedir = '/home2/szl'
    datadir = join(basedir, 'ICVL/val_list')
    newdir = join(basedir, 'sert/data/ICVL/icvl_val_512')
    #fnames = os.listdir(datadir)
    f = open("./icvl_partition/val_list.txt")
    fnames = f.readlines()
