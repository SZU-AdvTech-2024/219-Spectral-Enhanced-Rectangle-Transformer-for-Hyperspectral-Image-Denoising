...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=0, qk_scale=None, memory_blocks=128, down_rank=16,
                 weight_factor=0.1, attn_drop=0., proj_drop=0., split_size=1):
        super().__init__()
        self.dim = dim
        ...
        self.c_attns = CAB(dim, compress_ratio=4, squeeze_factor=down_rank, memory_blocks=memory_blocks)  # 构建 SE
...
class SMSBlock(nn.Module):
    """
    Residual Transformer Layer（RTL）构建类
    """
...
class SERT(nn.Module):
    """
    inp_channels：输入通道数
    dim：基础通道数，对应论文模型中的 C=96
    window_sizes：RA中矩形长边的大小
    depths：每层RTL中 Transformer Block 的数量
    num_heads：
    split_sizes：RA中矩形短边的大小
    mlp_ratio：
    down_rank：
    memory_blocks：
    qkv_bias：
    ...
    def __init__(self,
        ...
        for i_layer in range(len(self.num_layers)):
            # 构建 Residual Transformer Layer（RTL）
        ...
        self.conv_delasta = nn.Conv2d(dim, inp_channels, 3, 1, 1)  # last conv
